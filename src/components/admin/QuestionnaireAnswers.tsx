...
export default function QuestionnaireAnswers({ answers, universityId }: QuestionnaireAnswersProps) {
  const [approvalState, setApprovalState] = useState<Record<string, boolean>>(() => (
    answers.reduce((acc, answer) => {
      const initial = typeof answer.is_approved === "boolean" ? answer.is_approved : true;
  ...
  const sortedGroupedAnswers = useMemo(() => (
    Object.entries(groupedAnswers).sort(([firstDimension], [secondDimension]) => {
      ...
      {sortedGroupedAnswers.map(([dimension, dimensionAnswers]) => {
        ...
        return (
          <div key={dimension} className="card p-6 border-2 border-[#0047AB]/20">
                  ...
                  {dimensionAnswers.map((answer, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3 px-4 text-center">
                          <span className="px-2 py-1 rounded font-medium bg-linear-to-r from-[#f0f4ff] to-white text-[#000080] border-2 border-[#0047AB]/30">
                            {scoreValue}
                          </span>
                        </td>
                        <td className="py-3 px-4 text-center">
                          <span className={`px-2 py-1 rounded font-bold ${
                            isApproved
                              ? "bg-linear-to-r from-green-100 to-green-200 text-green-800 border-2 border-green-400"
            ...
            <div className="mt-4 p-4 bg-linear-to-r from-[#0047AB] to-[#0099ED] rounded-lg">
      ...
      <div className="sticky bottom-0 bg-white border-2 border-[#0047AB]/30 rounded-lg p-6 shadow-lg">
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            disabled={isSaving}
            className="px-8 py-3 bg-linear-to-r from-[#0047AB] to-[#0099ED] text-white rounded-lg hover:from-[#0099ED] hover:to-[#0047AB] disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg hover:shadow-xl font-semibold text-lg"
          >
